...
def frequency_powers(x, n_power_features=40):
    fxx, pxx = signal.welch(x, sampling_rate)
    features = dict()
...
def fft_features(beat):
    pff = extract_fft(beat[:int(0.13 * sampling_rate)])
    rff = extract_fft(beat[int(0.13 * sampling_rate):int(0.27 * sampling_rate)])
    tff = extract_fft(beat[int(0.27 * sampling_rate):])
...
def median_heartbeat(thb):
    if len(thb) == 0:
        return np.zeros((int(0.6 * sampling_rate)), dtype=np.int32)
...
def heart_beats_features2(thb):
    ...
    PQ = means[:int(0.15 * sampling_rate)]
    ST = means[int(0.25 * sampling_rate):]

    QR = means[int(0.13 * sampling_rate):r_pos]
    RS = means[r_pos:int(0.27 * sampling_rate)]

    q_pos = int(0.13 * sampling_rate) + np.argmin(QR)
    s_pos = r_pos + np.argmin(RS)

    p_pos = np.argmax(PQ)
    t_pos = np.argmax(ST)

    t_wave = ST[max(0, t_pos - int(0.08 * sampling_rate)):min(len(ST), t_pos + int(0.08 * sampling_rate))]
    p_wave = PQ[max(0, p_pos - int(0.06 * sampling_rate)):min(len(PQ), p_pos + int(0.06 * sampling_rate))]
...
def get_features_dict(x):
    [ts, fts, rpeaks, tts, thb, hrts, hr] = ecg.ecg(signal=x, sampling_rate=sampling_rate, show=False)
