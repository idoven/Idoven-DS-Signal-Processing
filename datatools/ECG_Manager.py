...
class ECG_Manager:
    ...
    def compute_peaks(self, ecg: int, thresh_prop = 0.6):
        ...
        However, the R peaks are not replacing the already included ones in notes.
        ...
        if len(peaks_pos) < 4:
            peaks_pos, props = sig.find_peaks(-1*epoch, height = np.max(-1*epoch)*thresh_prop)
    ...
    def compute_hr(self, ecg: int):
        ...
        self.notes.heart_rate[ecg] = heart_rate # Annotation
        return heart_rate
    
    def compute_thb(self, ecg:int):
        """ Calculation of total heart beats by counting the number
        of detected peaks and annotate it"""
        
        peaks_pos = self.compute_peaks(ecg)/self.fs
        
        total_heart_beats = peaks_pos.shape[0]
        self.notes.total_heart_beats[ecg] = total_heart_beats # Annotation
